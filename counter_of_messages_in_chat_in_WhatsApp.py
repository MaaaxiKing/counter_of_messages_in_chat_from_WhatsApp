...
for line in lines:
    signature = re.match("(?=([1-9]|(1[0-2]))/([1-9]|([12][0-9])|(3[01]))/\d{2}, (([01][0-9])|(2[0-3])):[0-5][0-9] - ).+?(?=: .+)", line)
    if signature:
        ID = signature.group().replace(re.match("([1-9]|(1[0-2]))/([1-9]|([12][0-9])|(3[01]))/\d{2}, (([01][0-9])|(2[0-3])):[0-5][0-9]" + separator_between_time_and_ID, line).group(), '')
        count_of_messages += 1
        IDs.add(ID)
